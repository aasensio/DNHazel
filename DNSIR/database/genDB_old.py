...
# Hinode's wavelength axis is 112 in length. We will work with arrays of length 128
# because it is more convenient for the deep networks
hinode_lambda = np.loadtxt('wavelengthHinode.txt')
x = np.arange(112)
coeff = np.polyfit(x, hinode_lambda, 1)
x_new = np.arange(128) - 8
new_lambda = coeff[0] * x_new + coeff[1]

center = 6301.5080
initial = np.min(new_lambda - center) * 1e3
final = np.max(new_lambda - center) * 1e3
step = (new_lambda[1] - new_lambda[0]) * 1e3
...
def compute(pars):
    ...
    stokesOut = np.zeros((n_sizeblock,4,128))
...
if rank == 0:
    ...
    f = h5py.File('/scratch1/deepLearning/DNSIR/database/database_sir.h5', 'w')
