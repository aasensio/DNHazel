...
Contains
  ...
  Type (model_2comp) :: Guess_model, Trial_model, Errors, Trial_errors, Prev_model
  Type (Nodes_info) :: Nodes
  Real, Dimension (Params%n_data) :: Obs_profile, Syn_profile, Sigma, Trial_profile, Prev_profile
  ...
  Real, Dimension (Params%n_points*4+Params%n_data*2) :: rectmp
  Real :: Lambda, Chisq, NWChisq, Last_accepted_chisq, Last_accepted_nwchisq
  Real :: Regul, Pertur
  Character (Len=20) :: Suffix
  Logical :: FileExists
  Integer :: n_failed, n_iter, itest, iunit, iost1, recsize
...
!
  ...
  ! Prepare file for Andres Deep Learning
  itest=0
  FileExists=.True.
  Do while (FileExists)
     itest=itest+1
     Write(suffix,'("_",i0)') itest
     Inquire(File='inv_'//suffix,Exist=FileExists)
  End do
  recsize=Params%n_points*4+Params%n_data
  Call Open_file_direct(iunit, 'inv_'//suffix, recsize*RealBytes)
  
  Do while ( .not. StopIter )
     Prev_profile=Syn_profile
     Prev_model=Guess_model
...
!
     If (Chisq .lt. Last_accepted_chisq .or. Adjusted) then
        ...
        Call Compute_dchisq_dx(Params, Line, Region, Nodes, Brute_force, &
             ...
             DchiDx, D2ChiD2x)
        n_failed=0
        ! Write models and profiles to Deep learning file

        rectmp(1:Params%n_points)=Prev_model%comp1%ltau_500
        rectmp(Params%n_points+1:2*Params%n_points)=Prev_model%comp1%temp
        rectmp(2*Params%n_points+1:3*Params%n_points)=Prev_model%comp1%v_mic
        rectmp(3*Params%n_points+1:4*Params%n_points)=Prev_model%comp1%v_los
        rectmp(4*Params%n_points+1:4*Params%n_points+Params%n_data)=Prev_profile
        rectmp(4*Params%n_points+1+Params%n_data+1:4*Params%n_points+2*Params%n_data)=Obs_profile
        Call write_direct(LittleEndian,iunit,rectmp, recsize, iost1)
        rectmp(1:Params%n_points)=Trial_model%comp1%ltau_500
        rectmp(Params%n_points+1:2*Params%n_points)=Trial_model%comp1%temp
        rectmp(2*Params%n_points+1:3*Params%n_points)=Trial_model%comp1%v_mic
        rectmp(3*Params%n_points+1:4*Params%n_points)=Trial_model%comp1%v_los
        rectmp(4*Params%n_points+1:4*Params%n_points+Params%n_data)=Trial_profile
        rectmp(4*Params%n_points+1+Params%n_data+1:4*Params%n_points+2*Params%n_data)=Obs_profile
        Call write_direct(LittleEndian,iunit,rectmp, recsize, iost1)        
     Else
        Lambda=Lambda*10
     ...
     n_iter=n_iter+1
  End do
  Call Close_File(iunit) ! Close Andres file
  Params%Skip_lambda=1 
