...
import time
import models_recursive as nn_model
...
# Only allocate needed memory
        ...
        self.model = nn_model.mfbd(64, 64, 3)
        self.model.summary()
    ...
    def training_generator(self):
        ...
        indices = np.arange(self.batchs_per_epoch_training)
        indices = np.roll(indices, np.random.randint(self.batchs_per_epoch_training))
        
        while 1:        
            for i in range(self.batchs_per_epoch_training):

                input_train = f[0].data[i*self.batch_size:(i+1)*self.batch_size,:,:,1:].astype('float32') / self.median
                output_train = f[0].data[i*self.batch_size:(i+1)*self.batch_size,:,:,0:1].astype('float32') / self.median
    ...
    def validation_generator(self):
        ...
        while 1:        
            for i in range(self.batchs_per_epoch_validation):
                ...
                input_validation = f[0].data[i*self.batch_size:(i+1)*self.batch_size,:,:,1:].astype('float32') / self.median
                output_validation = f[0].data[i*self.batch_size:(i+1)*self.batch_size,:,:,0:1].astype('float32') / self.median
    ...
    def define_network(self):
        ...
        if (self.network_type == 'encdec'):
            self.model = nn_model.encdec(self.nx, self.ny, self.noise, self.depth, activation=self.activation, n_filters=self.n_filters, n_inputs=self.n_inputs)

        if (self.network_type == 'keepsize'):
            self.model = nn_model.keepsize(self.nx, self.ny, self.noise, self.depth, activation=self.activation, n_filters=self.n_filters, n_inputs=self.n_inputs)
...
if (__name__ == '__main__'):
    ...
    parser.add_argument('-a','--action', help='Action', choices=['start', 'continue'])
    parser.add_argument('-i','--ninputs', help='N. inputs', default=12)
