...
from keras.layers import Input, Convolution3D, merge, Activation, Lambda, BatchNormalization
...
class trainDNNFull(object):
    ...
    def __init__(self, root, noise, option):
        ...
        self.input_file_images_training = "../database/database_images.h5"
        self.input_file_images_validation = "../database/database_images_validation.h5"
    ...
    def training_generator(self):
        ...
        while 1:        
            for i in range(self.batchs_per_epoch_training):
                ...
                input_train = images[i*self.batch_size:(i+1)*self.batch_size,1:,:,:,0].astype('float32')
                output_train = images[i*self.batch_size:(i+1)*self.batch_size,0:1,:,:,0].astype('float32')
    ...
    def validation_generator(self):
        ...
        while 1:        
            for i in range(self.batchs_per_epoch_validation):
                ...
                input_validation = images[i*self.batch_size:(i+1)*self.batch_size,1:,:,:,0].astype('float32')
                output_validation = images[i*self.batch_size:(i+1)*self.batch_size,0:1,:,:,0].astype('float32')
    ...
    def residual(self, inputs):
        x = ConvLSTM2D(self.n_filters, 3, 3, border_mode='same', init='he_normal', return_sequences=True)(inputs)
        x = BatchNormalization()(x)
        x = Activation('relu')(x)
        x = ConvLSTM2D(self.n_filters, 3, 3, border_mode='same', init='he_normal', return_sequences=True)(x)
        x = BatchNormalization()(x)
    ...
    def defineNetwork(self):
        ...
        conv = Convolution3D(self.n_filters, 3, 3, 3, activation='relu', border_mode='same', init='he_normal')(inputs)

        x = self.residual(conv)
        for i in range(self.n_conv_layers):
            x = self.residual(x)

        x = Convolution3D(self.n_filters, 3, 3, border_mode='same', init='he_normal')(x)
        x = BatchNormalization()(x)
        x = merge([x, conv], 'sum')

        final = Convolution3D(1, 1, 1, 1, activation='relu', border_mode='same', init='he_normal')(x)
