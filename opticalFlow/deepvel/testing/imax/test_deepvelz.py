...
from keras.layers import Input, Convolution2D, merge, Activation, Lambda, BatchNormalization, add
...
# Only allocate needed memory
    ...
    def predict_validation(self):
        ...
        db = ff.create_dataset("velocity", (self.n_frames, self.nx, self.ny, 3), dtype='float32')

        for i in range(3):
            out[:,:,:,i] = out[:,:,:,i] * (max_v[i+6] - min_v[i+6]) + min_v[i+6]
...
if (__name__ == '__main__'):
    ...
    out = trainDNNFull('../../training/cnns/deepvelz_10', '/net/vena/scratch1/deepLearning/opticalFlow/database/sf_Icon_307-364.sav', 'imax_velocity_noPmodes_vz.h5', 'mov')
    out.define_network()
