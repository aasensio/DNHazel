...
lower = np.asarray([0.0,      0.0,   0.0, -7.0, 0.0,  0.15, 0.15, 0.20,  1.0])
upper = np.asarray([3000.0, 180.0, 180.0,  7.0, 0.5,   1.2,  1.2, 0.80,  5.0])
nPar = 9
...
if rank == 0:
    ...
    f = h5py.File('/net/duna/scratch1/aasensio/deepLearning/milne/database/database_6301_hinode_1component.h5', 'w')
    
    databaseStokes = f.create_dataset('stokes', (nProfiles, nLambda, 4), 'f')
    databaseLambda = f.create_dataset('lambda', (nLambda,), 'f')
    databasePars = f.create_dataset('parameters', (nProfiles, nPar), 'f')
    ...
    while closed_workers < num_workers:
        ...
        if tag == tags.READY:
            ...
            databaseStokes[index*nSizeBlock:(index+1)*nSizeBlock,:,:] = stokes.T
            databasePars[index*nSizeBlock:(index+1)*nSizeBlock,:] = pars.T
