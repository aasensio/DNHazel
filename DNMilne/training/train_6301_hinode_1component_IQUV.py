...
from keras.layers import Input, Dense, Convolution1D, MaxPooling1D, Flatten, merge, GaussianNoise, BatchNormalization
...
# Stokes IQUV
        ...
        x = Convolution1D(nb_filter=self.nFeatures, filter_length=self.kernelSize, activation='relu', border_mode='valid', init='he_normal', name='conv_1')(stokes_input)
        x = BatchNormalization()(x)
        x = Convolution1D(nb_filter=self.nFeatures, filter_length=self.kernelSize, activation='relu', border_mode='same', init='he_normal', name='conv_2')(x)
        x = BatchNormalization()(x)
        x = MaxPooling1D(pool_length=self.poolLength, name='pool_1')(x)
        x = Convolution1D(nb_filter=self.nFeatures, filter_length=2*self.kernelSize, activation='relu', border_mode='same', init='he_normal', name='conv_3a')(x)
        x = BatchNormalization()(x)
        x = Convolution1D(nb_filter=self.nFeatures, filter_length=2*self.kernelSize, activation='relu', border_mode='same', init='he_normal', name='conv_3b')(x)
        x = BatchNormalization()(x)
        x = MaxPooling1D(pool_length=self.poolLength, name='pool_2')(x)
        x = Convolution1D(nb_filter=self.nFeatures, filter_length=3*self.kernelSize, activation='relu', border_mode='same', init='he_normal', name='conv_4a')(x)
        x = BatchNormalization()(x)
        x = Convolution1D(nb_filter=self.nFeatures, filter_length=3*self.kernelSize, activation='relu', border_mode='same', init='he_normal', name='conv_4b')(x)
        x = BatchNormalization()(x)
        x = MaxPooling1D(pool_length=self.poolLength, name='pool_3')(x)
        x = Convolution1D(nb_filter=self.nFeatures, filter_length=4*self.kernelSize, activation='relu', border_mode='same', init='he_normal', name='conv_5a')(x)
        x = BatchNormalization()(x)
        x = Convolution1D(nb_filter=self.nFeatures, filter_length=4*self.kernelSize, activation='relu', border_mode='same', init='he_normal', name='conv_5b')(x)
        x = BatchNormalization()(x)
        ...
        # self.model = Model(input=stokes_input, output=[out_BField, out_theta, out_chi, out_vmac,
