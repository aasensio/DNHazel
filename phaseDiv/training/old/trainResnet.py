...
from keras.layers import Input, Convolution2D, merge, Activation, Lambda, BatchNormalization, GaussianNoise
...
# Only allocate needed memory
    ...
    def defineNetwork(self):
        ...
        conv = Convolution2D(self.n_filters, 3, 3, activation='relu', border_mode='same', init='he_normal')(x)
    ...
    def trainCNN(self, n_iterations):
        ...
        self.metrics = self.model.fit_generator(self.training_generator(), self.n_training, nb_epoch=n_iterations, 
            callbacks=[self.checkpointer, self.history, self.tensorboard], validation_data=self.validation_generator(), nb_val_samples=self.n_validation)
